...
class TopicTokenizer(BaseTokenizer):
    def __init__(self):
    ...
    def tokenize(self, texts):
        splited_texts = []
        for text in texts:
            word_and_pos_list = self._tokenize_single_text(text)
            splited_texts.append(word_and_pos_list)
    ...
    def _tokenize_single_text(self, text):
        ...
        while node:
            ...
            if pos in ["名詞", "動詞", "形容詞", "感動詞", "助動詞", "副詞"]:
                ...
                word_list.append({
                  "word": mojimoji.han_to_zen(lemma),
                  "pos": pos,
                })
            node = node.next
        return word_list
