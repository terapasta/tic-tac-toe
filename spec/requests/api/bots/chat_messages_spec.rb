...
RSpec.describe '/api/bots/:token/chat_messages', type: :request do
  ...
  describe 'GET /api/bots/:token/chat_messages' do
    before do
      create_list(:message, 100, chat: chat)
    end

    it 'returns messages array' do
      get "/api/bots/#{bot.token}/chat_messages?guest_key=#{chat.guest_key}&per_page=20"
      JSON.parse(response.body).tapp do |json|
